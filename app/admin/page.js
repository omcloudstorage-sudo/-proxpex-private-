...
export default function AdminProjectsPage() {
  ...
  return (
    <div className="page-fade">
      ...
      <div className="bg-surface border border-line rounded-card shadow-card overflow-hidden mb-10 grid lg:grid-cols-[280px_1fr]">
        <ClientListCard
        ...
        <div className="p-6 md:p-8 border-t lg:border-t-0 border-line">
          <ClientGlobe
          ...
          <div className="mt-6 pt-6 border-t border-line">
            {selectedClient ? (
              ...
              <>
                <div className="text-xs font-medium text-slate uppercase tracking-wide mb-3">Client locations</div>
                {countryMarkers.length === 0 ? (
                  ...
                  <ul className="grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-1.5">
                    {countryMarkers.map((m) => (
                      <li key={m.id} className="flex items-center justify-between gap-3 text-sm text-slate">
                        <span className="truncate">{m.name}</span>
                        <span className="text-ink font-medium flex-shrink-0">{m.count}</span>
                      </li>
                    ))}
                  </ul>
                )}
...
function ClientListCard({ clients, selectedId, focusedProjectId, onSelectClient, onSelectProject }) {
  return (
    <div className="p-5">
      <div className="text-xs font-medium text-slate uppercase tracking-wide mb-3">Clients</div>
      {clients.length === 0 ? (
              ...
              <li key={c.id}>
                ...
                >
                  <span className="min-w-0 flex items-center gap-2">
                    <ChevronRight className={`w-3.5 h-3.5 flex-shrink-0 text-slate-light transition-transform ${active ? 'rotate-90' : ''}`} strokeWidth={2} />
                ...
                {active && c.projects.length > 0 && (
                  <ul className="ml-[1.4rem] mt-1 mb-1 space-y-0.5 border-l border-line pl-3">
                    {c.projects.map((p) => {
                        ...
                        <li key={p.id}>
                          <button
                            ...
                            className={[
                              ...
                              isFocused ? 'bg-signal-light text-signal font-medium' : 'text-slate hover:text-ink hover:bg-paper',
                            ].join(' ')}
                          >
                            <MapPin className="w-3 h-3 flex-shrink-0" strokeWidth={2} />
                            <span className="truncate flex-1">{p.name}</span>
                            <span className="text-slate-light flex-shrink-0">{p.countryName || 'No country'}</span>
                          </button>
                        </li>
                      )
                    })}
  ...
  return (
    <div>
      <div className="flex items-start justify-between gap-3 mb-3">
        <div>
          <div className="font-display text-lg font-semibold text-ink">{client.name}</div>
          <div className="text-sm text-slate flex items-center gap-1.5 mt-0.5">
            <MapPin className="w-3 h-3" strokeWidth={2} />
            {client.locationCount > 1 ? `${client.locationCount} locations` : client.countryName || 'No country set'}
          </div>
        </div>
        <button onClick={onClose} className="text-slate-light hover:text-ink flex-shrink-0" title="Close">
          <X className="w-4 h-4" strokeWidth={2} />
        </button>
      </div>

      {client.projects.length === 0 ? (
        <p className="text-sm text-slate-light">No projects for this client yet.</p>
      ) : (
          ...
          {client.projects.map((p) => (
            <Link
              ...
              className={[
                ...
                p.id === focusedProjectId ? 'bg-signal-light border-signal/40' : 'bg-paper hover:bg-signal-light/60 border-line',
              ].join(' ')}
            >
              <div className="min-w-0">
                <div className="text-sm font-medium text-ink truncate group-hover:underline">{p.name}</div>
                <div className="text-xs text-slate">PM: {p.pmName} {p.countryName ? `· ${p.countryName}` : ''}</div>
              </div>
              <div className="flex items-center gap-2 flex-shrink-0">
                <span className="text-xs font-bold text-signal">{p.pct}%</span>
                <ChevronRight className="w-3.5 h-3.5 text-slate-light group-hover:text-ink transition-colors" strokeWidth={2} />
              </div>
            </Link>
          ))}
        </div>
