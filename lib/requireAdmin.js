...
// Broader than requireProjectManager — anyone with view access to this
// specific project (Admin of its company, its assigned PM, a team member
// under that PM, or the client it belongs to), mirroring the `access` memo
// in app/project/[id]/page.js. Used for read-only document endpoints
// (view/thumbnail) where Client and Team member both need access but only
// Admin/PM can upload or delete.
export async function requireProjectAccess(req, projectId) {
  ...
  const allowed =
    (caller.role === 'admin' && project.companyId === caller.companyId) ||
    (caller.role === 'pm' && project.pmId === uid) ||
    (caller.role === 'team_member' && project.pmId === caller.pmId) ||
    (caller.role === 'client' && project.clientId === uid)
  if (!allowed) throw httpError(403, "You don't have access to this project.")
